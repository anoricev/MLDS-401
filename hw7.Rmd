...
```{r, warining=False}
library(psych)
pairs(train_data[, c("logcrim", "zn", "indus", "chas", "nox")])
pairs(train_data[, c("logcrim", "rm", "age", "dis", "rad")])
pairs(train_data[, c("logcrim", "tax", "ptratio", "black", "lstat")])
```
```{r}
...
for (var in predictors) {
  plot(boston[[var]], logcrim,
       xlab = var, ylab = "log(crim)",
       main = paste("log(crim) vs", var))
  lines(lowess(boston[[var]], logcrim), col="red", lwd=2)
}
par(mfrow = c(1,1))
```
...
Significant as F-statistic p-value is 0.01507 < 0.05. The residuals display a clear pattern in that they are linear with a positive slope. There is also a residual with a large Cook's distance, inidicating an issue. 
```{r}
plot(fit3)
```
...
This shows the limitations of the two iterative model selection methods.
